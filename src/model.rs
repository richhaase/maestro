...
impl Model {
    pub fn handle_permission_result(&mut self, status: PermissionStatus) {
        match status {
    ...
    pub fn apply_tab_update(&mut self, tabs: Vec<TabInfo>) {
    ...
    pub fn apply_pane_update(&mut self, update: PaneManifest) {
    ...
    pub fn handle_command_pane_opened(&mut self, pane_id: u32, ctx: BTreeMap<String, String>) {
        let title = ctx
            .get("pane_title")
            .cloned()
    ...
    pub fn handle_command_pane_rerun(&mut self, pane_id: u32, ctx: BTreeMap<String, String>) {
        let title = ctx
            .get("pane_title")
            .cloned()
            .unwrap_or_else(|| format!("maestro:{}", pane_id));
    ...
    pub fn handle_pane_closed(&mut self, pane_id: PaneId) {
    ...
    pub fn handle_key_event(&mut self, key: KeyWithModifier) {
        match self.mode {
