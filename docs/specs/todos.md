...
- [x] **UX: Tabbed sections with agent management**: Implemented tabbed interface (Tab key to switch) with "Maestro" section (running agent panes with type-down filter via 'f' key) and "Agents" section (agent management table with edit/delete/create). Fixed table column alignment issue by ensuring empty note fields use placeholder "—".
